...
interface LapTimeChartProps {
  data: Record<string, Array<{ lap_number: number; lap_time_seconds: number }>>;
}
...
export default function LapTimeChart({ data }: LapTimeChartProps) {
  ...
  const maxLap = Math.max(
    ...Object.values(data).flatMap((laps) => 
  ...
  const datasets = Object.entries(data).map(([driver, laps], index) => {
    ...
    laps.forEach((lap) => {
      if (lap.lap_number <= maxLap && lap.lap_time_seconds) {
        lapTimes[lap.lap_number - 1] = lap.lap_time_seconds;
      }
    });
